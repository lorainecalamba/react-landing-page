...
const Home = () => {
  return (
    <React.Fragment>
      ...
      <LeftPhotoCard title="Federal Holiday" photoImage={FederalPhotoImage} alt="Federal Image">
        <ul id="list-of-federal-holidays">
        {federalLinksList.map((list, index) => (
          <li 
            key={index}
            className={list.current ? 'current' : ''}>{list.holiday}</li>
        ))}
        </ul>
      </LeftPhotoCard>

      <RightPhotoCard title="Links" photoImage={LinkPhotoImage} alt="Links Image">
        <PhotoCardList arrayList={edgarLinksList}/>
      </RightPhotoCard>

      <LeftPhotoCard title="Tools" photoImage={FederalPhotoImage} alt="Federal Image">
